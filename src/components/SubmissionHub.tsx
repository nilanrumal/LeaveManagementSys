...
export default function SubmissionHub() {
  const [activeSubTab, setActiveSubTab] = useState<'overview' | 'requirements' | 'architecture' | 'usecases' | 'database'>('overview');
    ...
    <div className="space-y-8 print:p-0 print:bg-white print:shadow-none">
      ...
      <div className="flex flex-wrap gap-2 p-1.5 bg-slate-100 rounded-2xl print:hidden">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
            activeSubTab === 'database' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-600 hover:text-slate-800'
          }`}
        >
          <Database size={14} /> Database Design (ERD)
        </button>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm print:border-none print:shadow-none print:p-0">
        ...
        {(activeSubTab === 'database' || window.matchMedia('print').matches) && (
          <div className="space-y-8 print:block mt-8 print:mt-12">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Database className="text-orange-500" size={18} />
                <h3 className="font-sans font-black text-sm uppercase tracking-wider text-slate-800">5. NoSQL Firestore Database Design</h3>
              </div>
              <p className="text-xs text-slate-400">Schema layout of Google Cloud Firestore collections and entity mappings matching the live application.</p>
            </div>

            {/* NoSQL Schema diagram canvas */}
            <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 overflow-x-auto">
              <div className="min-w-[700px] grid grid-cols-5 gap-4 items-center relative">
                
                {/* COLUMN 1: users Collection */}
                <div className="col-span-2 space-y-4">
                  <div className="bg-white rounded-2xl border-2 border-slate-200 shadow-sm overflow-hidden">
                    <div className="bg-slate-800 text-white px-4 py-2.5 font-mono text-[11px] font-black uppercase tracking-wider flex justify-between items-center">
                      <span className="flex items-center gap-1.5">
                        <Users size={12} className="text-orange-400" /> users
                      </span>
                      <span className="text-[9px] bg-slate-700 px-2 py-0.5 rounded-full font-semibold">Collection</span>
                    </div>
                    <div className="p-3.5 font-mono text-[10px] space-y-2 divide-y divide-slate-100">
                      <div className="flex justify-between text-orange-600 font-bold pb-1">
                      ...
                      <div className="flex justify-between pt-1.5 font-semibold text-slate-700">
                        <span>role</span>
                        <span>String</span>
                      </div>
                      <div className="flex justify-between pt-1.5">
                      ...
                      <div className="flex justify-between pt-1.5">
                        <span>usedLeaveCount</span>
                        <span>Number</span>
                      </div>
                      <div className="flex justify-between pt-1.5 text-slate-400">
                ...
                {/* COLUMN 2: Logical Connector */}
                <div className="col-span-1 flex flex-col justify-center items-center text-slate-400 font-mono text-[11px] py-4">
                  <div className="text-[10px] font-black text-orange-600 bg-orange-100 px-2 py-0.5 rounded-full mb-1">1 .. N</div>
                  <div className="w-full flex items-center justify-center">
                    <span className="border-t-2 border-dashed border-slate-300 w-12" />
                    <span className="text-orange-500 font-black">&rarr;</span>
                    <span className="border-t-2 border-dashed border-slate-300 w-12" />
                  </div>
                  <div className="text-[9px] text-slate-400 mt-2 text-center leading-tight">
                    Linked via<br />
                    <span className="text-orange-600 font-bold">employeeId</span>
                  </div>
                </div>

                {/* COLUMN 3: leaveRequests Collection */}
                <div className="col-span-2 space-y-4">
                  <div className="bg-white rounded-2xl border-2 border-slate-200 shadow-sm overflow-hidden">
                    <div className="bg-slate-800 text-white px-4 py-2.5 font-mono text-[11px] font-black uppercase tracking-wider flex justify-between items-center">
                      <span className="flex items-center gap-1.5">
                        <FileText size={12} className="text-orange-400" /> leaveRequests
                      </span>
                      <span className="text-[9px] bg-slate-700 px-2 py-0.5 rounded-full font-semibold">Collection</span>
                    </div>
                    <div className="p-3.5 font-mono text-[10px] space-y-2 divide-y divide-slate-100">
                      <div className="flex justify-between text-slate-400 font-bold pb-1">
                        <span>🔑 id (Doc ID)</span>
                        <span>String</span>
                      </div>
                      <div className="flex justify-between pt-1.5 text-orange-600 font-bold">
                    ...
                    <div className="bg-slate-50 px-3 py-1.5 border-t border-slate-100 text-[9px] text-slate-500 font-sans italic">
                      * Denormalized fields for optimal performance
                    </div>
                  </div>
                </div>
            ...
            {/* Entity Relationships Explanation Card */}
            <div className="bg-slate-900 text-slate-100 rounded-3xl p-8 border border-slate-800 space-y-6 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-orange-500/10 rounded-full blur-3xl pointer-events-none" />
              <div className="flex items-center gap-2.5 border-b border-slate-800 pb-4">
                <ShieldCheck className="text-orange-500" size={20} />
                <h4 className="text-sm font-sans font-black uppercase tracking-wider">Entity Relationships</h4>
              </div>
              <div className="space-y-4 text-xs leading-relaxed text-slate-300">
                <p>
                  Since Firestore is a NoSQL database, traditional structural constraints (such as physical Foreign Keys) are not enforced on the engine level. Instead, a Logical One-to-Many (1:N) Relationship is managed at the application layer:
                </p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
                  <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800 space-y-2">
                    <span className="text-[10px] text-orange-400 font-mono font-bold uppercase tracking-wider block">1..N Relationship</span>
                    <p className="text-[11px]">
                      One User (<code className="text-orange-400">users</code>) can submit Many Leave Requests (<code className="text-orange-400">leaveRequests</code>).
                    </p>
                  </div>
                  <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800 space-y-2">
                    <span className="text-[10px] text-orange-400 font-mono font-bold uppercase tracking-wider block">Document Mapping</span>
                    <p className="text-[11px]">
                      The relation is linked using the <code className="text-orange-400">employeeId</code> key in the <code className="text-orange-400">leaveRequests</code> document, which matches the corresponding user's primary document ID (<code className="text-orange-400">uid</code>) in the <code className="text-orange-400">users</code> collection.
                    </p>
                  </div>
                  <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800 space-y-2">
                    <span className="text-[10px] text-orange-400 font-mono font-bold uppercase tracking-wider block">Denormalization</span>
                    <p className="text-[11px]">
                      Data Denormalization (such as copying <code className="text-orange-400">employeeName</code> and <code className="text-orange-400">employeeNo</code> directly into the <code className="text-orange-400">leaveRequests</code> collection) is intentionally practiced. This optimizes read times, minimizes joins, and ensures lightning-fast load times for historical lookups and PDF reports.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {/* Database Integrity Constraints Card */}
            <div className="bg-orange-50/40 rounded-3xl p-6 border border-orange-100 space-y-3">
              <h4 className="text-xs font-bold text-orange-800 flex items-center gap-1.5">
                <Award size={14} className="text-orange-600" /> Database Integrity &amp; Security Constraints
              </h4>
              <p className="text-xs text-slate-600 leading-relaxed">
                The database model secures key-integrity and guarantees high performance:
              </p>
              <ul className="list-none text-xs text-slate-600 space-y-2">
                <li className="flex items-start gap-2">
                  <span className="text-orange-500 font-bold">&bull;</span>
                  <span><strong>Referential Consistency:</strong> System-level deletion checks ensure acting assignments prevent orphan mappings or invalid user links.</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-orange-500 font-bold">&bull;</span>
                  <span><strong>Atomic Balance Updates:</strong> Approved leave counts immediately trigger automated decrement procedures across the applicant’s profile to keep figures accurate.</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-orange-500 font-bold">&bull;</span>
                  <span><strong>Granular Authorization Rules:</strong> Implemented security rules ensure write tasks are only allowed by authenticated profiles having appropriate clearance levels (HOD/CEO/Admin).</span>
                </li>
              </ul>
            </div>
          </div>
